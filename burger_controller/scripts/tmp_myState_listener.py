...
class MyStateBot(object):
    ...
    def __init__(self, mySide='r'):
        ...
        self.myStatePub = rospy.Publisher(
            'my_state', OverlayText, queue_size=5)
        self.warStateSub = rospy.Subscriber(
            '/war_state', war_state, self.warStateCallback)
        self.navStateSub = rospy.Subscriber(
            '/move_base/status', GoalStatusArray, self.navStateCallback)
        self.cvRectSub = rospy.Subscriber(
            '/cv_rect', CvRect, self.cvRectSubCallback)
        ...
        while not rospy.is_shutdown():
            try:
                ...
                self.my_state_text.text = self.my_state_text.text + "\n Rect R Area :" + \
                    str(self.rectData.rect_r.area) + "\t Width :" + str(
                        self.rectData.rect_r.length[0]) + "\t Height :" + str(self.rectData.rect_r.length[1])
                self.my_state_text.text = self.my_state_text.text + "\n Rect G Area :" + \
                    str(self.rectData.rect_g.area) + "\t Width :" + str(
                        self.rectData.rect_g.length[0]) + "\t Height :" + str(self.rectData.rect_g.length[1])
                self.my_state_text.text = self.my_state_text.text + "\n Rect B Area :" + \
                    str(self.rectData.rect_b.area) + "\t Width :" + str(
                        self.rectData.rect_b.length[0]) + "\t Height :" + str(self.rectData.rect_b.length[1])
    ...
    def pubTwistWithPIDController(self):
        ...
        self.isFoundEnemyTarget = enemy_target.center != (
            -1.0, -1.0) and enemy_target.length[1] > 100.0
        self.isFoundEnemy = self.isFoundEnemyTarget or (
            enemy.center != (-1.0, -1.0) and enemy.length[0] > 60.0)
        twist = Twist()

        if self.isFoundEnemyTarget:
            self.error = self.target_001 - enemy_target.center[0]
            self.integral += self.error * self.dt
            twist.angular.z = self.Kp * self.error + self.Kd * \
                (self.error - self.error_pre) / \
                self.dt  # + self.Ki * self.integral
